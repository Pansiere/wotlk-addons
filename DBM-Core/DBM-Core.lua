...
SlashCmdList["DEADLYBOSSMODS"] = function(msg)
	...
	elseif cmd:sub(1, 4) == "pull" then
		...
		local timer = 10
		local dpsTimer = 0
		local numbers = trim1(cmd:sub(6))
		local spacebarCharNum = numbers:find(" ")
		if (spacebarCharNum == nil) then
			timer = tonumber(numbers)
		else
			timer = tonumber(trim1(numbers:sub(0, spacebarCharNum - 1)))
			dpsTimer = tonumber(trim1(numbers:sub(spacebarCharNum + 1)))
		end
		...
		if (dpsTimer == 0) then
			...
			DBM:CreatePizzaTimer(timer + dpsTimer, DBM_CORE_TIMER_DPS, true)
			SendChatMessage(DBM_CORE_ANNOUNCE_PULL_FULL:format(timer, dpsTimer), channel)
			if timer > 7 then DBM:Schedule(timer - 7, SendChatMessage, DBM_CORE_ANNOUNCE_PULL_DPS_WAIT:format(7), channel) end
			if timer > 5 then DBM:Schedule(timer - 5, SendChatMessage, DBM_CORE_ANNOUNCE_PULL_DPS_WAIT:format(5), channel) end
			if timer > 3 then DBM:Schedule(timer - 3, SendChatMessage, DBM_CORE_ANNOUNCE_PULL_DPS_WAIT:format(3), channel) end
			if timer > 2 then DBM:Schedule(timer - 2, SendChatMessage, DBM_CORE_ANNOUNCE_PULL_DPS_WAIT:format(2), channel) end
			if timer > 1 then DBM:Schedule(timer - 1, SendChatMessage, DBM_CORE_ANNOUNCE_PULL_DPS_WAIT:format(1), channel) end
			DBM:Schedule(timer, SendChatMessage, DBM_CORE_ANNOUNCE_PULL_NOW_DPS_WAIT, channel)

			if dpsTimer > 7 then DBM:Schedule(timer + dpsTimer - 7, SendChatMessage, DBM_CORE_ANNOUNCE_DPS:format(7), channel) end
			if dpsTimer > 5 then DBM:Schedule(timer + dpsTimer - 5, SendChatMessage, DBM_CORE_ANNOUNCE_DPS:format(5), channel) end
			if dpsTimer > 3 then DBM:Schedule(timer + dpsTimer - 3, SendChatMessage, DBM_CORE_ANNOUNCE_DPS:format(3), channel) end
			if dpsTimer > 2 then DBM:Schedule(timer + dpsTimer - 2, SendChatMessage, DBM_CORE_ANNOUNCE_DPS:format(2), channel) end
			if dpsTimer > 1 then DBM:Schedule(timer + dpsTimer - 1, SendChatMessage, DBM_CORE_ANNOUNCE_DPS:format(1), channel) end
			
			DBM:Schedule(dpsTimer + timer, SendChatMessage, DBM_CORE_ANNOUNCE_DPS_NOW, channel)
		end
					...
					if not showedUpdateReminder then
						...
						if found then
							...
							if not DBM.Options.BlockVersionUpdatePopup then
								...
								DBM:AddMsg(("|HDBM:update:%s:%s|h|cff3588ff[http://kaster.zz.mu]"):format(displayVersion, revision))
							end
						end
					end
				end
