...
--Module variables
local playerCompatible, DrD_Font, updateSetItems, dmgMod
...
function DrDamage:GeneralOptions()
			...
			Actionbar = {
				...
				args = {
					...
					DisplayType_M = {
						...
						set =  function(info,v) settings["DisplayType_M"] = v; self:UpdateAB() end,
					},
					DisplayType_M2 = {
						type = 'select',
			...
			Tooltip = {
				...
				args = {
					...
					Hints = {
						...
						set = function(i, v) settings["Hints"] = v end,
					},
					Tooltip = {
						type = 'multiselect',
						name = L["Display"],
					...
					DefaultColor = {
						...
						set = function(i, v) settings["DefaultColor"] = v end,
					},
					TooltipTextColor1 = {
						type = 'color',
			...
			Calculation = {
				...
				args = {
					...
					HitTarget = {
						...
						set = function(info, v) settings["HitTarget"] = v; self:UpdateAB() end,
					},
					BossArmor = {
						type = 'toggle',
						name = L["Boss armor calculation"],
						desc = string_format(L["Automatically use %d armor for bosses"] .. ".", 10643),
						--descStyle = 'inline',
						order = 70,
						width = 'full',
						hidden = playerCaster,
						get = function() return settings["BossArmor"] end,
						set = function(info,v) settings["BossArmor"] = v; self:UpdateAB() end,
					},
					Armor = {
						type = 'range',
						name = L["Target armor"],
	...
	if playerCaster or playerHybrid then
		...
		tree["CompareDmg"] = L["Compare spell power"]
		tree["CompareCrit"] = L["Compare critical strike rating"]
		tree["CompareHit"] = L["Compare hit rating"]
		tree["CompareHaste"] = L["Compare haste rating"]
		...
		if not (playerClass == "DEATHKNIGHT") then
...
function DrDamage:ACTIONBAR_PAGE_CHANGED()
	if not self.fullUpdate then
		self.fullUpdate = self:ScheduleTimer("UpdateAB", 0.2)
	end
end

--Triggers after an ability is placed on the actionbar
function DrDamage:ACTIONBAR_HIDEGRID()
	if not self.fullUpdate and (GetCursorInfo() == "spell" or GetCursorInfo() == "macro") then
		self.fullUpdate = self:ScheduleTimer("UpdateAB", 0.2)
	end
end
	...
	for talentName, talentRank in pairs( talents ) do
		...
		for i=1,#talentTable do
			...
			if (melee and talent.Melee or talent.Caster and not melee or (not talent.Melee and not talent.Caster)) and not (talent.Specials and (baseSpell.AutoAttack or baseSpell.AutoShot)) then
				...
				if not DrD_MatchData(talent.Not,name) then
...
function DrDamage:GetSpellHit()
	local delta, playerLevel, _, boss = self:GetLevels()
	local player
	if settings.HitTarget and UnitExists("target") then
		...
		delta = settings.TargetLevel
		boss = (settings.TargetLevel == 3) and playerLevel >= 70
	end
	delta = math_max(-4,delta)
	return (player and hitDataPlayer[delta] or hitDataMOB[delta]) + hitMod + GetCombatRatingBonus(8), boss
end
...
AuraIterate = function(name, spell, auratable, bufftable)
	...
	for buff, data in pairs(bufftable) do
		if not data.Update and (caster and (data.Caster or not data.Melee) or melee and (data.Melee or not data.Caster)) then
			if not data.Not or data.Not and not DrD_MatchData(data.Not, spellName) then
				...
				or spell and (DrD_MatchData(buffschool, "Spells") or not healing and DrD_MatchData(buffschool, "Damage Spells"))
...
function DrDamage:HarvestValues(id, rk)
	...
	if not min then
		...
		min = min or string_match(GT:GetLine(GT:NumLines()),"Transfers (%d+) ")
		...
		min = min or string_match(GT:GetLine(GT:NumLines()),"that causes (%d+) ")
		min = min or string_match(GT:GetLine(GT:NumLines()),"and causes (%d+) ")
		min = min or string_match(GT:GetLine(GT:NumLines()),"for (%d+) every")
