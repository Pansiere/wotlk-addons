...
--GENERAL
	...
	self.Calculation["DEATHKNIGHT"] = function( calculation, ActiveAuras, Talents, spell, baseSpell )
		...
		if not calculation.healingSpell then
			if Talents["Black Ice"] and calculation.spellName ~= "Scourge Strike" then
...
--ABILITIES
	...
	self.Calculation["Attack"] = function( calculation, ActiveAuras, Talents )
		if Talents["Necrosis"] then
			local dmgM = calculation.dmgM_Magic * (ActiveAuras["Cinderglacier"] or 1)
		...
		if mrune == "3366" then
			...
			if target and string_find(undead,string_lower(target)) then
				calculation.extraDamBonus = 0.04 * (min+max) * calculation.dmgM_Magic
			else
				calculation.extraDamBonus = 0.02 * (min+max) * calculation.dmgM_Magic
			end
			calculation.extraDamage = 0
		elseif mrune == "3370" then
			local min, max = self:GetMainhandBase()
			calculation.extraDamage = 0
			calculation.extraDamBonus = 0.02 * (min+max) * calculation.dmgM_Magic * (ActiveAuras["Cinderglacier"] or 1) * (ActiveAuras["Frost Vulnerability"] or 1)
		end
		if calculation.offHand then
			local _, _, orune = string_split(":",GetInventoryItemLink("player",17))
			if orune == "3366" then
				local min, max = self:GetOffhandBase()
				local target = UnitCreatureType("target")
				if target and string_find(undead,string_lower(target)) then
					calculation.extraDamBonus_O = 0.04 * (min+max) * calculation.dmgM_Magic
				else
					calculation.extraDamBonus_O = 0.02 * (min+max) * calculation.dmgM_Magic
				end
				calculation.extraDamage = 0
			elseif orune == "3370" then
				local min, max = self:GetOffhandBase()
				calculation.extraDamage = 0
				calculation.extraDamBonus_O = 0.02 * (min+max) * calculation.dmgM_Magic * (ActiveAuras["Cinderglacier"] or 1) * (ActiveAuras["Frost Vulnerability"] or 1)
			end
		end
	...
	self.Calculation["Blood Strike"] = function( calculation, ActiveAuras )
		...
		if IsEquippedItem(39208) then
			calculation.minDam = calculation.minDam + 22.5 * diseaseCount
			calculation.maxDam = calculation.maxDam + 22.5 * diseaseCount
		end
		--Multiplicative - 3.3.3
	...
	self.Calculation["Scourge Strike"] = function( calculation, ActiveAuras, Talents )
		...
		if diseaseCount > 0 then
			--PTR: 0.25 --> 0.12
			--Multiplicative - 3.3.3
			calculation.dmgM = calculation.dmgM * (1 + diseaseCount * 0.25 * calculation.dmgM_Magic * (ActiveAuras["Cinderglacier"] or 1) * (1 + (Talents["Black Ice"] or 0)) * ((self:GetSetAmount( "T8 - Damage" ) >= 4) and 1.2 or 1))
...
--Melee
	--Sigil of the Dark Rider
	self.RelicSlot["Blood Strike"] = { 39208, 45 } --Applies 45 to base and 45 divided over 2(or 4?) diseases
	self.RelicSlot["Heart Strike"] = { 39208, 45 } --Applies 45 to base and 45 divided over 2(or 4?) diseases
	--Sigil of Awareness (base damage)
	self.RelicSlot["Obliterate"] ={ 40207, 336 }
	self.RelicSlot["Death Strike"] = { 40207, 315 }
	--Sigil of Awareness, Sigil of Arthritic Binding
	self.RelicSlot["Scourge Strike"] = { 40207, 292, 40875, 91.35, ModType2 = "finalMod" }
...
--Custom
	...
	--Bloody Vengeance
	self.PlayerAura[GetSpellInfo(50447)] = { School = { ["Damage Spells"] = true, ["Disease"] = true }, NoManual = true, ModType =
	...
	self.TargetAura[GetSpellInfo(51714)] = { Apps = 5, SelfCast = true, ID = 51714, ModType =
		function( calculation, ActiveAuras, _, _, apps )
			if calculation.school == "Frost" then
				calculation.dmgM = calculation.dmgM * (1 + 0.01 * apps)
			elseif calculation.spellName == "Attack" then
				ActiveAuras["Frost Vulnerability"] = 1 + 0.01 * apps
			end
		end
	}	
	--Crypt Fever
	...
	--FROST
		...
		[GetSpellInfo(49140)] = {	[1] = { Effect = 0.02, Spells = { "Frost", "Shadow", "Scourge Strike" }, ModType = "Black Ice"}, },
		--Nerves of Cold Steel
		[GetSpellInfo(49226)] = {	[1] = { Effect = 1, Melee = true, Spells = { "Physical", "Frost Strike" }, ModType = "Nerves of Cold Steel" },
									[2] = { Effect = 0.05, Melee = true, Spells = { "Attack", "Death Strike", "Obliterate", "Plague Strike", "Blood Strike", "Frost Strike" }, ModType = "offHdmgM", Multiply = true  },
									[3] = { Effect = 0.05, Melee = true, Spells = { "Death Strike", "Obliterate", "Plague Strike", "Blood Strike", "Frost Strike" }, ModType = "bDmgM_O", Multiply = true  }, NoManual = true },
		--Annihilation
		...
		[GetSpellInfo(49202)] = {	[1] = { Effect = 0.03, Specials = true, Spells = "All", Not = "Chains of Ice", ModType = "Tundra Stalker" }, },
	--UNHOLY
		--Vicious Strikes
		...
		[GetSpellInfo(50117)] = {	[1] = { Effect = 0.02, Spells = "All", Not = "Summon Gargoyle", ModType = "Rage of Rivendare" }, },
