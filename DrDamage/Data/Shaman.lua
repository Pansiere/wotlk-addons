	...
	self.Calculation["SHAMAN"] = function( calculation, ActiveAuras, Talents )
		...
		if calculation.healingSpell then
			...
			if name and string_find(elw,name) then
				...
				calculation.extra = (bonus + 0.455 * 1.88 * (12/15) * calculation.spellDmg) * (1 + (Talents["Purification"] or 0))
				calculation.extraTicks = 4
				calculation.extraName = name
...
--ABILITIES
	...
	self.Calculation["Attack"] = function( calculation, _, Talents, spell, baseSpell )
		...
		if name then
			...
			elseif string_find(ft, name) then
				...
				calculation.E_critM = 0.5 + (Talents["Elemental Fury"] or 0)
				calculation.extraDamBonus = calculation.extraDamBonus + (spd * bonus + coeff * (GetSpellBonusDamage(3) + calculation.spellDmg)) * calculation.dmgM_Magic
				calculation.extraChance = math_min(1,(self:GetSpellHit() + calculation.spellHit + (Talents["Elemental Precision"] or 0)) / 100)
				calculation.extraName = name
			elseif string_find(fb, name) then
				...
				calculation.E_critM = 0.5 + (Talents["Elemental Fury"] or 0)
				calculation.extraDamBonus = calculation.extraDamBonus + (bonus + 0.1 * (GetSpellBonusDamage(5) + calculation.spellDmg)) * calculation.dmgM_Magic
				calculation.extraChance = (spd * 9)/60 * ((self:GetSpellHit() + calculation.spellHit + (Talents["Elemental Precision"] or 0)) / 100)
				calculation.extraName = name
			end
		end
		if calculation.offHand then
			...
			if name then
				if string_find(wf, name) then
					...
					calculation.extraName = calculation.extraName and (calculation.extraName ~= name) and (calculation.extraName .. "+" .. name) or name
				elseif string_find(ft, name) then
					...
					calculation.E_critM = 0.5 + (Talents["Elemental Fury"] or 0)
					calculation.extraDamBonus_O = calculation.extraDamBonus + (spd * bonus + coeff * (GetSpellBonusDamage(3) + calculation.spellDmg)) * calculation.dmgM_Magic
					calculation.extraChance_O = math_min(1,(self:GetSpellHit() + calculation.spellHit + (Talents["Elemental Precision"] or 0)) / 100)
					calculation.extraName = calculation.extraName and (calculation.extraName ~= name) and (calculation.extraName .. "+" .. name) or name
				elseif string_find(fb, name) then
					local _, spd = self:GetWeaponSpeed()
					local bonus = tonumber(rank) or 9
					...
					calculation.E_critM = 0.5 + (Talents["Elemental Fury"] or 0)
					calculation.extraDamBonus_O = calculation.extraDamBonus + (bonus + 0.1 * (GetSpellBonusDamage(5) + calculation.spellDmg)) * calculation.dmgM_Magic
					calculation.extraChance_O = (spd * 9)/60 * ((self:GetSpellHit() + calculation.spellHit + (Talents["Elemental Precision"] or 0)) / 100)
					calculation.extraName = calculation.extraName and (calculation.extraName ~= name) and (calculation.extraName .. "+" .. name) or name
				end
			end
		end
	...
	self.Calculation["Chain Heal"] = function( calculation )
		...
		if self:GetSetAmount( "T10 Healer" ) >= 4 then
			...
			calculation.extraTicks = 3
			calculation.extraName = "4p T10"
		end
	end
	self.Calculation["Chain Lightning"] = function( calculation )
		--Totem of Hex: Unaffected by shamanism
		if IsEquippedItem(40267) then
			calculation.minDam = calculation.minDam + 165 * calculation.spellDmgM
			calculation.maxDam = calculation.maxDam + 165 * calculation.spellDmgM
		end
...
--RELIC
	...
	--Totem of the Void, Totem of the Storm, Totem of Ancestral Guidance
	self.RelicSlot["Lightning Bolt"] = { 28248, 55, 23199, 33, 32330, 85 }
	self.RelicSlot["Chain Lightning"] = { 28248, 55, 23199, 33, 32330, 85 }
...
--Custom
	...
	self.spellInfo = {
		...
		[GetSpellInfo(8050)] = {
			--DONE: BASE OK, INCREASE OK, COEFFICIENT OK, DOWNRANK OK
			["Name"] = "Flame Shock",
			[0] = { School = { "Fire", "Shock" }, sFactor = 0.5, canCrit = true, Cooldown = 6, eDuration = 18, sTicks = 3, BaseIncrease = true },
		...
		[GetSpellInfo(974)] = {
			--DONE: BASE OK, INCREASE OK, COEFFICIENT OK, DOWNRANK OK
			["Name"] = "Earth Shield",
			[0] = { School = { "Nature", "Healing", }, canCrit = true, sHits = 6, SPBonus = 3.228, NoDPS = true, NoDoom = true, NoDebuffs = true, NoPeriod = true, Downrank = 9 },
	...
	--ELEMENTAL:
		...
		[GetSpellInfo(30672)] = { 	[1] = { Effect = 1, Caster = true, Spells = { "Lightning Shield", "Nuke", "Shock", "OffensiveTotem", }, ModType = "hitPerc", },
									[2] = { Effect = 1, Melee = true, Spells = "Attack", ModType = "Elemental Precision" }, },
