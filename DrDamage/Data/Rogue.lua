...
--GENERAL
	...
	self.Calculation["ROGUE"] = function( calculation, ActiveAuras, Talents, spell, baseSpell )
		...
		if Talents["Find Weakness"] and ActiveAuras["Find Weakness"] then
			calculation.dmgM = calculation.dmgM * (1 + Talents["Find Weakness"])
		end
		if Talents["Master of Subtlety"] and ActiveAuras["Master of Subtlety"] then
			calculation.dmgM = calculation.dmgM * (1 + Talents["Master of Subtlety"])
		end
		if Talents["Dirty Deeds"] then
			if(UnitHealth("target") ~= 0) then
		...
		if buff or buffO then
			...
			hit =  0.01 * math_min(100, self:GetSpellHit() + calculation.spellHit)
		end
...
--TALENTS
	...
	local has = GetSpellInfo(13960)
	self.Calculation["Hack and Slash"] = function( calculation, value, baseSpell )
		--Requires One-Handed Swords
		local mhType, ohType = self:GetWeaponType()
		if not calculation.extraDamage then
			local mh = (mhType == ohs) or (mhType == oha)
			local oh = ((ohType == ohs) or (mhType == oha)) and (baseSpell.AutoAttack or baseSpell.OffhandAttack or baseSpell.DualAttack)
			if mh or oh then
				calculation.extraDamage = 0
				...
				calculation.extraName = has
			end
		end
	end
	self.Calculation["Serrated Blades"] = function( calculation, value )
		calculation.armorMod = calculation.armorMod - value * calculation.playerLevel
	end
--ABILITIES
	...
	self.Calculation["Sinister Strike"] = self.Calculation["Hemorrhage"]
	self.Calculation["Backstab"] = self.Calculation["Hemorrhage"]
--SETS
...
--Player
	...
	--Find Weakness
	self.PlayerAura[GetSpellInfo(31234)] = { ActiveAura = "Find Weakness", ID = 31234 }
	--Shadowstep
	self.PlayerAura[GetSpellInfo(36554)] = { ActiveAura = "Shadowstep", ID = 36554 }
	--Master of Subtlety
	self.PlayerAura[GetSpellInfo(31221)] = { ActiveAura = "Master of Subtlety", ID = 31221 }
	--Envenom
...
--Target
	...
	self.spellInfo = {
		...
		[GetSpellInfo(26679)] = {
			["Name"] = "Deadly Throw",
			[0] = { School = "Ranged", NoPoison = true, WeaponDamage = 1, ComboPoints = true },
		...
		[GetSpellInfo(51723)] = {
			["Name"] = "Fan of Knives",
			[0] = { WeaponDamage = 0.7, DualAttack = true, Cooldown = 10, AoE = true },
			[1] = { 0 },
		},
		[GetSpellInfo(51690)] = {
			["Name"] = "Killing Spree",
