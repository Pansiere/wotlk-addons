...
--ABILITIES
	...
	self.Calculation["Power Word: Shield"] = function( calculation, _, Talents )
		--PW: Shield - (1 + 0.05 * IPWS) * (1 + 0.02 * SpiritualHealing + 0.01 * Twin Disciplines + 0.02 * Focused Power)
		local pwsMod = self.healingMod * (1 + (Talents["Improved Power Word: Shield"] or 0)) * (1 + (Talents["Spiritual Healing"] or 0) + (Talents["Twin Disciplines"] or 0) + (Talents["Focused Power"] or 0))
		local auraMod = calculation.dmgM
		calculation.dmgM_custom = true
		calculation.dmgM = pwsMod
		--Glyph of Power Word: Shield
		if self:HasGlyph(55672) then
			local glyphMod = (1 + (Talents["Improved Power Word: Shield"] or 0) + (Talents["Twin Disciplines"] or 0) --[[+ (Talents["Spiritual Healing"] or 0)--]]) * (1 + (Talents["Blessed Resilience"] or 0))
			if not UnitExists("target") or UnitIsUnit("target","player") then glyphMod = glyphMod * (1 + (Talents["Focused Power"] or 0)) end
			...
			if Talents["Divine Aegis Bonus"] then
				--calculation.extraName = calculation.extraName .. "+" .. da
				calculation.extraCritEffect = (1 + Talents["Divine Aegis Bonus"]) * ((self:GetSetAmount( "Tier 9 - Healing" ) >= 4) and 1.1 or 1)
			end
		end
	end
...
--Custom
	...
	self.spellInfo = {
		...
		[GetSpellInfo(33076)] = {
					--BASE OK, INCREASE X, COEFFICIENT OK, DOWNRANK OK
		...
		[GetSpellInfo(724)] = {
					...
					["Name"] = "Lightwell",
					[0] = { School = { "Holy", "Healing" }, Cooldown = 180, SPBonus = 1.88, eDot = true, eDuration = 6, sTicks = 2, Stacks = 10, NoDownrank = true },
	...
	--HOLY:
		...
		--Blessed Resilience (multiplicative - 3.3.3)
		[GetSpellInfo(33142)] = {	[1] = { Effect = 0.01, Spells = "Healing", Not = "Lightwell", Multiply = true, },
									[2] = { Effect = 0.01, Spells = "Power Word: Shield", ModType = "Blessed Resilience" }, },
	...
	--DISCIPLINE:
		...
		--Focused Power (multiplicative, additive on PW:S - 3.3.3)
		[GetSpellInfo(33190)] = {	[1] = { Effect = 0.02, Spells = "All", Multiply = true },
									[2] = { Effect = 0.02, Spells = "Power Word: Shield", ModType = "Focused Power" }, },
		--Soul Warding
		...
		--Divine Aegis
		[GetSpellInfo(47515)] = {	[1] = { Effect = 0.1, Spells = "Healing", ModType = "Divine Aegis", },
